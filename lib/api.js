...
const links = 'https://freerestapi.herokuapp.com'
...
const ytmp3 = async (url) => new Promise((resolve, reject) => {
    axios.get(`${links}/api/v1/yt2?url=${url}`)
    .then((res) => {
		resolve(res.data)
    })
    .catch((err) =>{
...
const fb = async (url) => new Promise((resolve, reject) => {
	axios.get(`${links}/api/v1/fb?url=${url}`)
	.then((res) => {
		if (res.status !== 200) resolve({status: 'error'})
		resolve({jud: res.data.title, linksd: res.data.rawVideo, linkhd: res.data.streamURL})
	})
	.catch((err) =>{
        reject(err)
    })
...
/*
const stalkigpict = async (username) => new Promise((resolve, reject) => {
    axios.get(`${links}/api/v1/igs?u=${username}`)
    .then((res) => {
        if (res.data.status != 200) resolve("Error 404");
        resolve(res.data.profilehd);
    })
    .catch((err) =>{
        reject(err)
    })
...
const stalkig = async (username) => new Promise((resolve, reject) => {
    axios.get(`${links}/api/v1/igs?u=${username}`)
    .then((res) => {
        if (res.data.status != 200) resolve("Error 404");
        const text = `User: ${res.data.data.username}\nName: ${res.data.data.fullname}\nBio: ${res.data.data.Bio}\nFollowers: ${res.data.data.followers}\nFollowing: ${res.data.data.following}\nPost: ${res.data.data.timeline}`
        resolve(text)
    })
    .catch((err) =>{
        reject(err)
...
const nhentai = async (doujinid) => new Promise((resolve, reject) => {
	axios.get(`${links}/api/v1/nhentai?d=${doujinid}`)
	.then((res) => {
		if (res == undefined) return reject("Tidak Ada Hasil")
		const hsl = `Judul: ${res.data.title}\n\nPages: Karena Kebanyakan Foto Jadi Baca Sendiri di https://nhentai.net/`+doujinid
		console.log(hsl.replace(/,/g,"\n"));
		resolve({status: 'success', hasil: hsl.replace(/,/g,"\n"), cov: res.data.cover})
	})
	.catch((err) => {
		reject(err)
	})
...
const igdl = async (url) => new Promise((resolve, reject) => {
	axios.get(`${links}/api/v1/ig?url=${url}`)
	.then((res) => {
		if (res.code != 200) return resolve({status: "error", hasil: 500})
		resolve({file: res.file});
	})
	.catch((err) => {
		reject(err);
	})
